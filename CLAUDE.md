...
## SummonAI Kit Context Engine

Generated by the new SummonAI Kit context engine. Use these artifacts as codebase memory, not as frozen library documentation.

### Context Rule

- Use `ctx7 library` and `ctx7 docs` for library, framework, SDK, CLI, and cloud-service basics.
- Use generated skill references for repository-specific patterns, gotchas, local files, and side-effect ordering.
...
- implementer: Implements changes using Context7 for library basics and generated skills for repo-specific patterns.
- verifier: Requires real evidence, command output, screenshots, hashes, and gate checks before work can advance.
- reviewer: Reviews code for bugs, regressions, security issues, missing tests, and mismatch with repo patterns.
- closer: Packages final evidence, PR notes, changed files, and remaining risks after verification succeeds.
- retro: Analyzes run logs and failures to suggest harness improvements without mutating memory automatically.
